...
public class MaxIncreasetoKeepCitySkyline_807 {
    ...
    public int maxIncreaseKeepingSkyline(int[][] grid) {
        int[] hline = new int[grid[0].length];
        int[] vline = new int[grid.length];

        int temp;
        for (int x=0;x<vline.length;++x){
            for (int y=0;y<hline.length;++y){
                temp = grid[x][y];
                if (temp>vline[x]) vline[x]=temp;
                if (temp>hline[y]) hline[y]=temp;
            }
        }
        ...
        for (int x=0;x<vline.length;++x){
            for (int y=0;y<hline.length;++y){
                res += Math.min(vline[x], hline[y]) - grid[x][y];
            }
        }
        return res;
    }
