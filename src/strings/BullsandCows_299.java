...
public class BullsandCows_299 {
    ...
    public String getHint(String secret, String guess) {
        ...
        for (int i=0;i<s.length;++i){    // if not exact match, use positive to record times of appearance in secret
            if (s[i]==g[i]) ++a;         // use negtive to record times of appearance in guess
            else {
                if (map[s[i]-'0']<0) ++b; 
                if (map[g[i]-'0']>0) ++b;
                ++map[s[i]-'0'];
