...
public class SecondMinimumNodeInaBinaryTree_671 {
    ...
    public int findSecondMinimumValue(TreeNode root) {
        ...
        int rl = root.left.val==root.val? findSecondMinimumValue(root.left):root.left.val;
        int rr = root.right.val==root.val? findSecondMinimumValue(root.right):root.right.val;
        if (rl<0 && rr<0) return -1;
        if (rl>0 && rr>0) return rl<rr? rl:rr;
        return rl<0? rr:rl;
    }
}
