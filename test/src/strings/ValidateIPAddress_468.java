...
using two consecutive colons (::) to pursue simplicity. For example, 2001:0db8:85a3::8A2E:0370:7334 is an invalid IPv6 address.
...
public class ValidateIPAddress_468 {
    ...
    public String validIPAddress(String IP) {
        ...
        if (v4.length==4) {
            return v4Validate(v4)? "IPv4":"Neither";
        }        
        else if (v6.length==8) {
            return v6Validate(v6)? "IPv6":"Neither";
        }
        else return "Neither";
    }

    private boolean v6Validate(String[] v6) {        
        for (String i:v6) {
    ...
    private boolean v4Validate(String[] v4) {
        for (String i:v4) {
            int cur=0;
