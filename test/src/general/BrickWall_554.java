...
public class BrickWall_554 {
    ...
    public int leastBricks(List<List<Integer>> wall) {
        ...
        for (List<Integer> row:wall) {
            int pos=0;
            for (int i=0;i<row.size()-1;++i) {   // last brick ends on right edge of the wall, don't include that
                pos+=row.get(i);
                if (edge.containsKey(pos)) edge.put(pos, edge.get(pos)+1);
                else edge.put(pos, 1);
                if (edge.get(pos)>max) max=edge.get(pos);
            }
        }
